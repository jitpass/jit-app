...
/// One line an owner group states once for all its rows ("Made by …, now
/// gone", "Launched by …"), and the file it names, for Show in Finder.
...
/// lot: a broken launcher's advice is the same for every row of one
/// launcher kind, so it follows them rather than repeating under each.
...
public extension DoctorGroup {
    ...
    /// The rows in runs: broken launchers by launcher kind in first-seen
    /// order, as `jit doctor` prints them, each closed by its advice; any
    /// other kind one run with no note.
    var runs: [DoctorRun] {
        guard kind == "launcher_broken" else {
        ...
        return order.map { key in
            let rows = byKind[key] ?? []
            return DoctorRun(items: rows, note: DoctorAdvice.brokenLauncherAdvice(rows))
        }
    }
}

public extension DoctorAdvice {
    /// jit's advice for one broken launcher, singular, to the plural it
    ...
    /// What to do about a run of broken launchers of one kind, said once:
    ...
    static func brokenLauncherAdvice(_ rows: [DoctorItem]) -> String? {
    ...
    /// names none (a vault path is not a file). A broken launcher's file,
    /// then its launcher's; a pointer file, never the pointed-at secret; an
    /// owner or unlaunched row's profile manifest; the config, binary,
    ...
    static func filePath(_ item: DoctorItem) -> String? {
        ...
        case "pointer_missing":
            item.file
        case "owner_gone", "no_owner", "unlaunched", "mcp", "mcp_nested", "jit_path", "jit_path_upgrade",
